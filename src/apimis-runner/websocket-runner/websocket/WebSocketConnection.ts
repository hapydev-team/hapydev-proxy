...
import BufferList from './FastBufferList';
...
var setImmediateImpl =
  'setImmediate' in global ? global.setImmediate.bind(global) : process.nextTick.bind(process);

var idCounter = 0;

export function WebSocketConnection(socket, extensions, protocol, maskOutgoingPackets, config) {
  ...
  if (this.config.keepalive && !this.config.useNativeKeepalive) {
    ...
    if (this.config.dropConnectionOnKeepaliveTimeout) {
      if (typeof this.config.keepaliveGracePeriod !== 'number') {
        throw new Error(
          'keepaliveGracePeriod  must be specified and ' +
...
function validateCloseReason(code) {
  ...
  if (code >= 1000 && code <= 2999) {
    ...
    return (
      [1000, 1001, 1002, 1003, 1007, 1008, 1009, 1010, 1011, 1012, 1013, 1014, 1015].indexOf(
        code
      ) !== -1
    );
  }
