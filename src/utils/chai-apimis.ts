import Ajv from 'ajv';
import { forOwn, get, has, isEqual, isNumber, isPlainObject, isString } from 'lodash';
...
const ChaiApimis = function (chai) {
  const Assertion = chai.Assertion;
  forOwn(statusClassReasons, (classCode, reason) => {
  ...
  forOwn(statusCodeReasons, (reason, assertionName) => {
    Assertion.addProperty(assertionName, function () {
      ...
      this.assert(
        actualStatusCode === reason.statusCode,
  ...
  Assertion.addProperty('ok', function () {
    this.assert(chai.util.flag(this, 'object'), '#{this} 应为真', '#{this} 应为假');
    return;
  });

  Assertion.addMethod('status', function (codeOrReason) {
    if (isNumber(codeOrReason)) {
      new Assertion(this._obj).to.have.property('statusCode');
      new Assertion(this._obj.statusCode).to.be.a('number');

      this.assert(
        this._obj.statusCode === Number(codeOrReason),
        '预期响应的状态代码应为 #{exp}，但实际的响应为 #{act}',
        '预期响应没有状态代码 #{act}',
        Number(codeOrReason),
    ...
    if (isString(codeOrReason)) {
      new Assertion(this._obj).to.have.property('statusMessage');
      this.assert(
        this._obj.statusMessage === codeOrReason,
        '预期响应的状态应为 #{exp}，但实际的响应为 #{act}',
        '预期响应没有状态 #{act}',
        codeOrReason,
        this._obj.statusMessage
      );
    }
  });
  ...
  Assertion.addMethod('header', function (headerKey, headerValue) {
    ...
    const _map = [];
    Object.keys(this._obj.headers).forEach((key) => {
      _map.push([key, this._obj.headers[key]]);
    });
    const _headers = new Map(_map);

    this.assert(
      _headers.has(headerKey),
  ...
  Assertion.addProperty('withBody', function () {
    ...
    this.assert(isString(bodyText) && bodyText.length, '响应体应不为空', '响应体应为空');
  });
  ...
  Assertion.addMethod('body', function (content) {
    ...
    if (isPlainObject(content)) {
      ...
      try {
        bodyData = this._obj.json();
      } catch (e) {}

      this.assert(
        isEqual(bodyData, content),
    ...
    console.log(this._obj, 22222);

    new Assertion(this._obj).to.have.property('rawBody');
    new Assertion(this._obj.rawBody).to.be.a('string');
    bodyData = this._obj.rawBody;

    if (arguments.length === 0) {
      this.assert(isString(bodyData) && bodyData.length, '响应体应不为空', '响应体应为空');
      return;
    }

    if (content instanceof RegExp) {
      this.assert(
        content.exec(bodyData) !== null,
  ...
  Assertion.addMethod('jsonBody', function (path, value) {
    ...
    let jsonBody, parseError;

    try {
      jsonBody = this._obj.json();
    } catch (e) {
      if (e.name === 'JSONError' && e.message) {
        parseError = e.message.replace(FIRST_LINE, '$1');
      } else {
    ...
    if (isString(path)) {
      ...
      if (arguments.length > 1) {
        jsonBody = get(jsonBody, path);
        this.assert(
          isEqual(jsonBody, value),
  ...
  Assertion.addChainableMethod(
    ...
    function () {
      ...
      this._obj = get(this._obj, 'responseTime');
    }
  );
  ...
  Assertion.addMethod('jsonSchema', function (schema, options) {
    ...
    this.assert(
      ...
      'expected data to not satisfy schema',
      true,
      valid
    );
  });
